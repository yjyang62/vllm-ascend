...
def _update_compilation_modes(vllm_config: VllmConfig, ascend_config) -> None:
    ...
    if model_config and hasattr(model_config.hf_text_config, "index_topk"):
        ...
        additional_config.setdefault(
            ...
            get_ascend_device_type() == AscendDeviceType.A5 and requested_cache_dtype == "bfloat16",
        )
        logger.info(
            "DeepSeek-V4 attention KV mode: %s (requested cache dtype: %s)",
            "BF16 SparseFlashMla" if additional_config[DSV4_EXPLICIT_BF16_KV_KEY] else "main-compatible FP8",
