...
SPARSE_FLASH_MLA_REQUIRED_Q_HEADS = 64
...
def _bf16_pad_q_heads_for_sparse_flash_mla(
    q: torch.Tensor,
    kwargs: dict[str, Any],
) -> tuple[torch.Tensor, int]:
    """Pad local TP heads to the current sparse_flash_mla N1 contract.

    The A5 sparse_flash_mla operator documents N1 == 64 for both TND and BSND
    query layouts. DSV4 TP>1 passes only local heads (for example 16 heads at
    TP4), so pad the unused head lanes before the op and crop them afterwards.
    """
    layout_q = kwargs.get("layout_q", "BSND")
    head_axis = _bf16_q_head_axis(q, layout_q)
    local_heads = q.shape[head_axis]
    if local_heads == SPARSE_FLASH_MLA_REQUIRED_Q_HEADS:
        return q, local_heads
    if local_heads <= 0 or local_heads > SPARSE_FLASH_MLA_REQUIRED_Q_HEADS:
        raise ValueError(
            f"sparse_flash_mla requires 1..{SPARSE_FLASH_MLA_REQUIRED_Q_HEADS} q heads before padding, "
            f"got {local_heads}."
        )

    pad_shape = list(q.shape)
    pad_shape[head_axis] = SPARSE_FLASH_MLA_REQUIRED_Q_HEADS - local_heads
    ...
    if isinstance(sinks, torch.Tensor) and sinks.dim() == 1 and sinks.shape[0] == local_heads:
        kwargs["sinks"] = torch.cat((sinks, sinks.new_zeros(SPARSE_FLASH_MLA_REQUIRED_Q_HEADS - local_heads)), dim=0)
    return q, local_heads


def _bf16_crop_sparse_flash_mla_output(out: Any, local_heads: int, layout_q: str) -> Any:
    if local_heads == SPARSE_FLASH_MLA_REQUIRED_Q_HEADS or not isinstance(out, torch.Tensor):
        return out
...
def _bf16_sparse_flash_mla_metadata(**kwargs):
    ...
    if 0 < kwargs.get("num_heads_q", 0) < SPARSE_FLASH_MLA_REQUIRED_Q_HEADS:
        kwargs["num_heads_q"] = SPARSE_FLASH_MLA_REQUIRED_Q_HEADS
...
def _bf16_sparse_flash_mla(*args, **kwargs):
    ...
    q, local_heads = _bf16_pad_q_heads_for_sparse_flash_mla(args[0], kwargs)
    result = torch.ops._C_ascend.npu_sparse_flash_mla(q, *args[1:], **kwargs)
    if isinstance(result, tuple):
        return (_bf16_crop_sparse_flash_mla_output(result[0], local_heads, kwargs.get("layout_q", "BSND")), *result[1:])
