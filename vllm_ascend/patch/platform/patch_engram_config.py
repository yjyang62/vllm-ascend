...
PR nightlies skip image rebuild and can still ship vLLM 0.29, which has no
``vllm.config.engram``. In that case this module is a no-op so ``adapt_patch()``
does not fail pytest collection for unrelated models.
"""
