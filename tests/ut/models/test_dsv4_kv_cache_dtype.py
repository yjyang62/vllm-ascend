...
from vllm_ascend.models.layer.attention.layer import (
    dsv4_indexer_kv_dtype,
    dsv4_resolve_attn_kv_dtype,
    dsv4_uses_a5_bf16_kv,
)
...
@pytest.mark.parametrize(
    ("device_type", "cache_dtype", "expected_attn", "expected_indexer", "uses_bf16"),
    [
        (AscendDeviceType.A5, "bfloat16", torch.bfloat16, torch.float8_e4m3fn, True),
        (AscendDeviceType.A5, "float8_e4m3fn", torch.float8_e4m3fn, torch.float8_e4m3fn, False),
        (AscendDeviceType.A2, "bfloat16", torch.bfloat16, torch.int8, False),
        (AscendDeviceType.A2, "auto", torch.bfloat16, torch.int8, False),
    ],
)
def test_dsv4_a5_attn_kv_dtype_respects_request(
    ...
    with patch(
        ...
        assert dsv4_uses_a5_bf16_kv(vllm_config) is uses_bf16
        assert dsv4_resolve_attn_kv_dtype(vllm_config, torch.bfloat16) == expected_attn
        assert dsv4_indexer_kv_dtype() == expected_indexer
