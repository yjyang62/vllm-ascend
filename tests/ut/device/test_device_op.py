...
def _patch_sharedkv_ops():
    """UT env may not ship compiled _C_ascend sparse sharedkv ops."""
    return mock.patch.multiple(
        torch.ops._C_ascend,
        npu_kv_quant_sparse_attn_sharedkv=mock.DEFAULT,
        npu_kv_quant_sparse_attn_sharedkv_metadata=mock.DEFAULT,
        npu_sparse_attn_sharedkv=mock.DEFAULT,
        npu_sparse_attn_sharedkv_metadata=mock.DEFAULT,
        create=True,
    )
...
def test_a5_dsa_attn_kv_plan_centralizes_sparse_flash_mla_choices():
    ...
    with (
        ...
        a2_plan = BaseDeviceAdaptor.build_dsa_attn_kv_plan(torch.bfloat16)
    assert a2_plan.uses_sparse_flash_mla is False
    assert a2_plan.layout_kv == "PA_ND"
    assert a2_plan.requires_block_offset_slots is True
    assert a2_plan.pack_kv_head_dim_extra is False
    assert a2_plan.compressor_slot_mapping_format == DSA_COMPRESSOR_SLOT_MAPPING_BLOCK_OFFSET

    with mock.patch(
        "vllm_ascend.attention.dsa_attn_kv_plan.get_ascend_device_type",
        return_value=AscendDeviceType.A3,
    ):
        a3_plan = BaseDeviceAdaptor.build_dsa_attn_kv_plan(torch.bfloat16)
    assert a3_plan.uses_sparse_flash_mla is True
    assert a3_plan.layout_kv == "PA_BBND"
    assert a3_plan.sparse_attn_op is sparse_flash_mla
    assert a3_plan.sparse_attn_metadata_op is sparse_flash_mla_metadata
    assert a3_plan.metadata_kwargs("npu:0") == {"device": "npu:0"}
