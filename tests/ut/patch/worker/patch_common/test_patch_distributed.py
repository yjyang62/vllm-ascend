...
def test_hccl_sleep_destroy_and_restore_unshared_group(module_env):
    module_env.utils_module.create_hccl_pg_options.return_value = {
        "hccl_config": {"hccl_buffer_size": 200},
        "non_default_field": 7,
    }
    ...
    assert group._acquired_hccl_keys == []
    assert len(group._unshared_hccl_groups) == 1

    assert group.destroy_hccl_for_sleep() is True

    assert module_env.destroy_process_group.call_args_list == [call(original_device_group)]
    assert group.device_group is None
    assert group._unshared_hccl_groups == []
    ...
    assert group.device_group is not original_device_group
    assert group._acquired_hccl_keys == []
    assert group._unshared_hccl_groups == [group.device_group]


def test_top_level_hccl_sleep_helpers_count_alive_groups_once(module_env):
    first = _make_group(
        module_env,
        group_ranks=[[0, 1]],
        group_name="tp",
    ...
    original_device_group = first.device_group

    assert first.device_group is second.device_group

    assert module_env.module.destroy_hccl_for_sleep() == 2

    assert first.device_group is None
    assert second.device_group is None
    assert module_env.destroy_process_group.call_args_list == [call(original_device_group)]

    assert module_env.module.destroy_hccl_for_sleep() == 0
    assert module_env.module.restore_hccl_after_sleep() == 2

    assert first.device_group is not None
