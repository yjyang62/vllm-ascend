...
| `experimental_hccp_lease` | bool | `false` | Same-device mode. Opt-in prototype that keeps the HCCP service allocated across extra-cleanup sleep by deferring `rtCloseNetService` in a process-local preload shim. Requires `enabled` and `sleep_mode_extra_cleanup`, plus `LD_PRELOAD` of `csrc/sleep_hccp_lease/net_lease.c`. Extra-cleanup still destroys and restores business HCCL groups. This is not a supported public CANN API. |
