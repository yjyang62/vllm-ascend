...
| `sleep_mode_extra_cleanup` | bool | `false` | Same-device mode. Enables HCCL process-group release + ACL graph workspace cleanup during sleep, returning more NPU memory to the trainer at the cost of increased wakeup latency. Extra cleanup keeps the HCCP service allocated via a process-local `LD_PRELOAD` shim built from `csrc/sleep_hccp_lease/net_lease.c`; missing native symbols fail closed. This option is available only under `rl_config`; the former top-level `enable_sleep_mode_extra_cleanup` key has been removed. |
