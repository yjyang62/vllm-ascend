...
| `sleep_mode_extra_cleanup` | bool | `false` | Same-device mode. Enables HCCL process-group release + ACL graph workspace cleanup during sleep, returning more NPU memory to the trainer at the cost of increased wakeup latency. One usable multi-rank HCCL group is kept as a CANN HCCP/AICPU lifecycle anchor (prefer TP; otherwise DP/PP/EP/world). Teardown is skipped only when no such group exists. This option is available only under `rl_config`; the former top-level `enable_sleep_mode_extra_cleanup` key has been removed. |
