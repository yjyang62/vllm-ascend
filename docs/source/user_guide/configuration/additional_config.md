...
| `sleep_mode_extra_cleanup` | bool | `false` | Same-device mode. Enables HCCL process-group release + ACL graph workspace cleanup during sleep, returning more NPU memory to the trainer at the cost of increased wakeup latency. The multi-rank `world` HCCL group is kept as a CANN HCCP/AICPU lifecycle anchor; teardown is skipped when `world` is missing or single-rank. This option is available only under `rl_config`; the former top-level `enable_sleep_mode_extra_cleanup` key has been removed. |
