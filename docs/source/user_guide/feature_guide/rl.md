...
| --- | --- |
| `sleep_mode_extra_cleanup` | Release HCCL process groups and ACL graph workspaces during sleep in a same-NPU deployment; keeps the HCCP service allocated via the preload shim |
...
| `rl_config.sleep_mode_extra_cleanup=true` | Optional for sleep mode | Release HCCL and ACL graph resources at the cost of slower wake-up; requires `LD_PRELOAD` of the HCCP lease shim |
