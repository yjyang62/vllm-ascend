...
- waits for pending pipeline-parallel send work, synchronizes the NPU, and destroys HCCL process groups while keeping the multi-rank `world` group as a CANN HCCP/AICPU lifecycle anchor. Skip HCCL teardown when `world` is missing or single-rank.
