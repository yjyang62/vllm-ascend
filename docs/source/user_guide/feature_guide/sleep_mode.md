...
- waits for pending pipeline-parallel send work, synchronizes the NPU, and destroys HCCL process groups while keeping one usable multi-rank group (prefer TP) as a CANN HCCP/AICPU lifecycle anchor.
