...
- waits for pending pipeline-parallel send work, synchronizes the NPU, and destroys HCCL process groups while keeping a usable multi-rank TP group as a CANN HCCP/AICPU lifecycle anchor. If no such TP group exists, HCCL teardown is skipped for that sleep cycle.
